...
1. **Search first.** Найдите существующий use case, policy, validation schema и тест через `rg`. Проверьте официальную документацию, если API или версия могли измениться.
2. **Опишите риск.** Для auth, billing, документов, PII, migrations и публичной видимости перечислите failure modes до редактирования.
3. **Закрепите поведение.** Для исправления сначала воспроизведите дефект тестом. Для новой функции зафиксируйте happy path, отказ и ролевую границу.
4. **Внесите минимальное связное изменение.** Route Handlers держите тонкими; транзакцию и policy размещайте в правильном слое.
