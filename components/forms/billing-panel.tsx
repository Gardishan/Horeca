...
export function BillingPanel({ plans, invoices, currentPlanCode, betaDemoOnly = false }: { plans: Plan[]; invoices: Invoice[]; currentPlanCode?: string; betaDemoOnly?: boolean }) {
  ...
  async function jsonAction(key: string, endpoint: string, body: object = {}) {
    ...
    setMessage(response.ok ? "Операция выполнена" : payload.error?.message ?? "Ошибка"); setError(!response.ok); setPending(""); if (response.ok) { if (key === "paid") setBetaPaymentAcknowledged(false); router.refresh(); }
  }
