...
export function VerificationPanel({ acceptedTypes, documents, betaDemoOnly = false, submissionLocked = false }: { acceptedTypes: string[]; documents: DocumentItem[]; betaDemoOnly?: boolean; submissionLocked?: boolean }) {
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="surface-flat flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-extrabold">3. Отправить на проверку</h2><p className="mt-1 text-sm text-slate-500">{submissionLocked ? "Компания проверена и активна. Повторная отправка недоступна: она скрыла бы товары из каталога до новой проверки." : "Система автоматически проверит профиль, согласия, документы, тариф и оплату."}</p></div>{submissionLocked ? null : <Button disabled={pending !== ""} onClick={() => post("submit", "/api/dashboard/company/submit-verification")}><Send className="size-4" />{pending === "submit" ? "Проверяем…" : "Отправить компанию"}</Button>}</section>
    </div>
  );
}
