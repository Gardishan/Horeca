...
export default async function CompanyDashboardPage() {
  ...
  return (
    <div className="grid gap-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Профиль заполнен" value={`${data.profile.percent}%`} icon={Users} /><MetricCard label="Опубликовано" value={data.publishedProducts} note={maxProducts === null ? "Без лимита" : `из ${maxProducts ?? 0}`} icon={Package} tone="blue" /><MetricCard label="Тариф" value={data.subscription?.plan.code ?? "—"} note={data.subscription ? formatMoney(data.subscription.plan.priceMonthly.toString()) : "Не выбран"} icon={CreditCard} tone="amber" /><Link href="/dashboard/requests" aria-label={`Открыть заявки покупателей: ${data.company._count.buyerRequests}`} className="rounded-2xl hover:ring-2 hover:ring-brand-700"><MetricCard label="Заявки" value={data.company._count.buyerRequests} note="Открыть заявки" icon={FileCheck2} /></Link></div>
