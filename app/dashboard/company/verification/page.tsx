...
export default async function VerificationPage() {
  ...
  return <div className="grid gap-7"><PageHeader eyebrow="Trust & Safety" title="Верификация поставщика" description="Примите юридические документы, загрузите реквизиты и отправьте компанию на ручную проверку." actions={<StatusBadge status={data.verificationStatus} />} /><VerificationPanel acceptedTypes={data.legalAcceptances.filter((item) => item.version === LEGAL_VERSION).map((item) => item.type)} documents={documents} betaDemoOnly={isBetaDemoOnly()} /></div>;
}
