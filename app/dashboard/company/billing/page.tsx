...
export default async function BillingPage() {
  ...
  const pending = data.subscriptions.find((subscription) => subscription.status === "PENDING_PAYMENT");
  ...
  return (
    <div className="grid gap-7"><PageHeader eyebrow="Биллинг" title="Тариф и оплата" description="Выберите тариф, сформируйте счёт и загрузите подтверждение банковского перевода." /><Alert tone="warning" title="Ручная оплата MVP">На MVP-этапе оплата подтверждается вручную администратором. В контролируемой Beta это только демонстрационный сценарий, не подтверждение реального платежа. Для production потребуется платёжный шлюз и фискальные/бухгалтерские интеграции.</Alert><BillingPanel plans={plans} invoices={invoices} currentPlanCode={current?.plan.code} currentSubscriptionId={current?.id} pendingSubscription={pending ? { id: pending.id, planCode: pending.plan.code } : undefined} betaDemoOnly={isBetaDemoOnly()} />
