...
- активация: незаблокированная компания, полный профиль, согласия, все текущие документы `APPROVED` и с допустимым antivirus verdict (`REJECTED`/`REUPLOAD_REQUESTED` документ перестаёт учитываться только после более нового upload того же типа), одобренный `REGISTRATION` или `BIN_IIN`, payment `CONFIRMED` и verification `APPROVED`;
- `ACTIVE` компания не может повторно отправить себя на проверку; снятие блокировки возвращает компанию в `DRAFT`/`PENDING_REVIEW` и требует повторной активации.
...
В development/test документы могут храниться в `storage/private/company-documents`; путь исключён из Git и никогда не отдаётся как static content. Staging/production fail-fast требуют S3-compatible private object storage. Upload проверяет размер, расширение, MIME и magic bytes; имя генерируется через UUID. Администратор скачивает объект только через защищённый API, каждое успешное скачивание создаёт `DocumentDownloadLog`.

`antivirusCheck()` использует deterministic mock только в dev/test. Staging/production требуют HTTPS scanner по `docs/MALWARE_SCAN_BACKEND.md`; outage, timeout, malformed response и unknown verdict блокируют upload до storage. S3 boundary, failure semantics и provider evidence описаны в `docs/PRIVATE_OBJECT_STORAGE.md`. Для закрытия production control всё ещё нужны реально развёрнутые scanner/bucket, IAM/KMS/lifecycle/retention/quarantine policy и staging evidence.
